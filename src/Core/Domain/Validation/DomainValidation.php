...
class DomainValidation
{
    ...
    {
        if (empty($value)) throw new EntityValidationException($exceptMessage ?? "Should not empty or null");
    }
    ...
    {
        if (!empty($value) && strlen($value) > $length) throw new EntityValidationException($exceptMessage ?? "The value must not be greater than {$length} characters");
    }
}
